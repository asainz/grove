...
await (commands[command] ?? createSession)(commands[command] ? rawArgs : [command, ...rawArgs])
...
async function cleanupSession(name: string, mode: CleanupMode) {
  ...
  if (!shouldDelete) {
    console.log(`Keeping worktree '${name}'. Resume with: grove resume ${name}`)
    return
  }
...
function resolveWorktree(name: string) {
  ...
  if (result.error) {
    exitWithError(`Failed to locate worktree '${name}': ${result.error.message}`)
  }
